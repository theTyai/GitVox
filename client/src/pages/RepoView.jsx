...
function RepoView() {
  ...
  const addCollab = async () => {
    try {
      ...
      alert("Added!"); setCollaborator('');
    } catch (err) { alert(err.response.data.error); }
  };

  if (!repoData) return <div className="text-white p-8">Loading...</div>;
    ...
    <div className="flex h-screen bg-gray-900 text-white overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-700 bg-gray-800">
          ...
          {repoData.repo.owner === user._id && (
            <div className="mt-4 flex gap-2">
              <input value={collaborator} onChange={(e) => setCollaborator(e.target.value)} placeholder="GitHub Username" className="w-full bg-gray-900 p-2 rounded text-xs"/>
              <button onClick={addCollab} className="bg-green-600 px-3 rounded text-xs font-bold">+</button>
            </div>
          )}
        </div>
        <div className="overflow-y-auto flex-1 p-2 space-y-2">
          {repoData.commits.map(c => (
             <div key={c.sha} onClick={() => setSelectedCommit(c)} className={`p-3 rounded cursor-pointer border transition ${selectedCommit?.sha === c.sha ? 'border-purple-500 bg-gray-800' : 'border-gray-700 hover:bg-gray-800'}`}>
      ...
      <div className="flex-1 flex flex-col bg-gray-900">
        {selectedCommit ? (
          ...
          <div className="flex-1 flex items-center justify-center text-gray-500">Select a commit</div>
        )}
      </div>
    </div>
  );
